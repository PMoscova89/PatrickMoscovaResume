...
final class AVFoundationCoordinatorAudioVideoRouteTests: XCTestCase {
    ...
    func test_showVideo_routesToMediaPlayerVC() {
        let host = HostVC()
        let nav = InterceptNav(rootViewController: host)
        ...
        // Simulate tapping "Play Video"
        featureController.performUserAction(.videoTapped)
        
        // The coordinator should push a MediaPlayerViewController
        XCTAssertTrue(nav.lastPushed is MediaPlayerViewController,
                      "Expected MediaPlayerViewController to be displayed for video route")
    }
}
